...
function resizeApp(){
	if(!begin_entered){
	   ...
	   scale = (((winWidth/appWidth)<(winHeight/appHeight))?(winWidth/appWidth):(winHeight/appHeight))-0.01//-0.15; //scaling
			...
			var resContWidth = $("#responsive_container").width()*scale-0.01;
			console.log(resContWidth);
			if(resContWidth >= 350 && resContWidth <= 450){
				$('.swiper-pagination-bullet').css({"width":"20px","height":"20px"});
				$('.swiper-pagination-clickable .swiper-pagination-bullet').css({"margin-left":"41px"});
			}else{
				$('.swiper-pagination-bullet').css({"width":"10px","height":"10px"});
				$('.swiper-pagination-clickable .swiper-pagination-bullet').css({"margin-left":"21px"});
			}
